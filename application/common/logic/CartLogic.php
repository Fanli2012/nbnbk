...
class CartLogic extends BaseLogic
{
    ...
    public function getAll($where = array(), $order = '', $field = '*', $limit = '')
    {
		...
		{
			$res[$k]['is_promote'] = 0;
			if( model('Goods')->bargain_price($v['goods_price'],$v['promote_start_date'],$v['promote_end_date']) > 0 ){ $res[$k]['is_promote'] = 1; }
    ...
    public function add($data = array(), $type=0)
    {
        ...
        if (!$goods) {return ReturnData::create(ReturnData::PARAMS_ERROR,null,'商品不存在');}
        ...
        if($cart_id){return ReturnData::create(ReturnData::SUCCESS, $cart_id, '购物车添加成功');}
