...
class OrderLogic extends BaseLogic
{
    ...
    public function add($data = array(), $type=0)
    {
		...
		if($data['user_bonus_id'] > 0)
		{
			...
			if($user_bonus){$discount = $discount + $user_bonus['bonus_money'];}
		}
		...
		foreach($order_goods['list'] as $k=>$v)
		{
			...
			if($v['goods_number'] > $goods['goods_number']){ Db::rollback(); return ReturnData::create(ReturnData::SYSTEM_FAIL, null, '商品库存不足'); }
    ...
    public function orderYuepay($where = array())
    {
		...
		if($user_money['code'] != ReturnData::SUCCESS){ Db::rollback(); return ReturnData::create(ReturnData::FAIL, null, $user_money['msg']); }
    ...
    public function orderSetComment($where = array())
    {
        ...
        $order = $this->getModel()->getOne($where);
        if(!$order){return ReturnData::create(ReturnData::PARAMS_ERROR,null,'订单不存在或已评价');}
