...
class UserReferralCommissionLogic extends BaseLogic
{
    ...
    {
        if ($money <= 0) { return ReturnData::create(ReturnData::PARAMS_ERROR, null, '金额必须大于零'); }
        ...
        if(!$user_referral_commission){return ReturnData::create(ReturnData::PARAMS_ERROR, null, '用户推介资金不存在');}
        //判断可提取佣金是否足够
        if ($money > $user_referral_commission['commission_available']) { return ReturnData::create(ReturnData::PARAMS_ERROR, null, '可提取佣金不足'); }
        Db::startTrans(); //启动事务
        //修改用户推介资金
