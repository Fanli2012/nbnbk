...
class Bonus extends Validate
{
    ...
    protected $rule = [
        ...
        ['money', 'require|regex:/^\d{0,10}(\.\d{0,2})?$/', '优惠券金额必填|优惠券金额格式不正确'],
		...
		['get_time', 'require|number|max:11', '添加时间必填|添加时间格式不正确|添加时间格式不正确'],
    ];
    
    protected $scene = [
        'add'  => ['name', 'money', 'min_amount', 'start_time', 'end_time', 'num', 'point', 'status', 'get_time'],
        'edit' => ['name', 'money', 'min_amount', 'start_time', 'end_time', 'num', 'point', 'status', 'get_time'],
        'del'  => ['id'],
    ];
}