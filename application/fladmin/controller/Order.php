...
class Order extends Base
{
    ...
    {
		if(isset($_POST['id']) && !empty($_POST['id'])){$id = $_POST['id'];}else{exit(json_encode(ReturnData::create(ReturnData::PARAMS_ERROR)));}
		
        $data['shipping_id'] = input('shipping_id', '');
        $data['shipping_sn'] = input('shipping_sn', '');
        
        if($data['shipping_id'] == ''){exit(json_encode(ReturnData::create(ReturnData::PARAMS_ERROR)));}
        ...
        {
            if($data['shipping_sn'] == ''){exit(json_encode(ReturnData::create(ReturnData::PARAMS_ERROR)));}
    ...
    public function change_status()
    {
		if(isset($_POST['id']) && !empty($_POST['id'])){$id = $_POST['id'];}else{exit(json_encode(ReturnData::create(ReturnData::PARAMS_ERROR)));}
		$status = input('status', '');
        if($status==''){exit(json_encode(ReturnData::create(ReturnData::PARAMS_ERROR)));}
        ...
        elseif($status==7)
        {
            ...
            //返库存
            if(!model('Order')->returnStock($id)){exit(json_encode(ReturnData::create(ReturnData::FAIL)));}
        }
        elseif($status==8)
        {
            ...
            if(!$order){exit(json_encode(ReturnData::create(ReturnData::PARAMS_ERROR)));}
            if($order['pay_money']>0)
            {
                ...
                if($user_money['code'] != ReturnData::SUCCESS){ exit(json_encode(ReturnData::create(ReturnData::FAIL))); }
            }
            
            //返库存
            if(!Order::returnStock($id)){exit(json_encode(ReturnData::create(ReturnData::FAIL)));}
        }
